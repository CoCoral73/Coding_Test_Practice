...
let T = fIO.readInt()
for _ in 0..<T {
    let (w, h) = (fIO.readInt(), fIO.readInt())
    var maze = [[Character]](), isVisited = [[Int]](repeating: [Int](repeating: Int.max, count: w), count: h)
    let J_Queue = Deque<RC>(), F_Queue = Deque<RC>()
    for i in 0..<h {
        let line = [Character](fIO.readString())
        maze.append(line)
        for j in 0..<w {
            if line[j] == "@" { J_Queue.push_back((i, j)); isVisited[i][j] = 0 }
            else if line[j] == "*" { F_Queue.push_back((i, j)); isVisited[i][j] = 0 }
        }
    }
    ...
    while !J_Queue.isEmpty {
        ...
        for _ in 0..<J_cnt {
            ...
            for i in 0..<4 {
                let (rr, cc) = (r + dr[i], c + dc[i])
                if rr < 0 || rr >= h || cc < 0 || cc >= w { answer = min(answer, isVisited[r][c] + 1); continue }
                if maze[rr][cc] == "#" || maze[rr][cc] == "*" { continue }
                if isVisited[rr][cc] <= isVisited[r][c] + 1 { continue }
                isVisited[rr][cc] = isVisited[r][c] + 1
